...
# ./zig-out/bin/wisp export|perl -MJSON::XS -nE '$z=decode_json($_); for my $t (@{$z->{tags}}) { say $t->[1] if $t->[0] eq "e"}'|sort|uniq -c|sort -rn|head -50|perl -nE '/\d+\s+(\w+)/ && say $1'
...
sub testScan {
    ...
    my $resA = `./zig-out/bin/wisp export --db wisp-db-test/wisp.db 2>/dev/null | perl test/dumbFilter.pl '$fge' $headCmd | jq -r .id | sort | sha256sum`;
    my $resB = `./zig-out/bin/wisp export --db wisp-db-test/wisp.db 2>/dev/null | jq -r .id | sort | sha256sum`;
...
if ($cmd eq 'scan') {
    ...
    die "unknown cmd: $cmd\nUsage: perl test/filterFuzzTest.pl [scan|scan-limit]";
}
