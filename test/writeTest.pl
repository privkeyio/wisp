...
sub doTest {
    ...
    {
        open(my $fh, '-|', './zig-out/bin/wisp export --db wisp-db-test/wisp.db 2>/dev/null') || die "$!";
        while(<$fh>) {
    ...
    # Sort both arrays to compare (wisp may export in different order than strfry)
    my @expectedIds = sort map { $eventIds->[$_] } @{$spec->{verify}};
    my @gotIds = sort @$finalEventIds;

    for (my $i = 0; $i < @gotIds; $i++) {
        die "id mismatch: expected $expectedIds[$i] got $gotIds[$i]" if $expectedIds[$i] ne $gotIds[$i];
    }
}


sub cleanDb {
    system("mkdir -p wisp-db-test");
    system("rm -f wisp-db-test/wisp.db wisp-db-test/wisp.db-lock");
}

sub addEvent {
    my $ev = shift;

    system(qq{ test/nostril $ev >test-eventXYZ.json });

    my $eventJson = `cat test-eventXYZ.json`;

    system(qq{ <test-eventXYZ.json ./zig-out/bin/wisp import --db wisp-db-test/wisp.db 2>/dev/null });
